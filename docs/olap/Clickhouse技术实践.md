...
```
![image.png](https://cdn.nlark.com/yuque/0/2023/png/390265/1703066121911-d549b1be-2d41-4dfd-9748-223e60d102f3.png#averageHue=%23fdfdfd&clientId=u46d9de2a-b1d3-4&from=paste&height=279&id=ufc0447db&originHeight=279&originWidth=708&originalType=binary&ratio=1&rotation=0&showTitle=false&size=49528&status=done&style=none&taskId=ueadb4659-b8c8-49ae-8219-ab1ba6fbc17&title=&width=708)<br />整个执行计划从下至上大致分成两个步骤：<br />1）查询各个分片数据<br />在图10-18所示执行计划中，One和Remote步骤是并行执行的，它们分别负责了本地和远端分片的查询动作。其中，在One步骤会将SQL转换成对本地表的查询：
```sql
...
<a name="nD8r1"></a>
## Muation突变
ClickHouse提供了DELETE和UPDATE的能力，这类操作被称为Mutation查询，它可以看作ALTER语句的变种。虽然Mutation能最终实现修改和删除，但不能完全以通常意义上的UPDATE和DELETE来理解，我们必须清醒地认识到它的不同：首先，Mutation语句是一种“很重”的操作，更适用于批量数据的修改和删除；其次，**它不支持事务，一旦语句被提交执行，就会立刻对现有数据产生影响**，无法回滚；最后，Mutation语句的执行是一个异步的后台过程，语句被提交之后就会立即返回。所以这并不代表具体逻辑已经执行完毕，它的具体执行进度需要通过system.mutations系统表查询。<br />主键不能更新。<br />有一张person_update表。原来的表分区结构<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/390265/1703643882444-f95d6570-1286-4134-8ee9-ee389a3b3d11.png#averageHue=%232c2d2a&clientId=u833a1b5e-6319-4&from=paste&height=81&id=u8dc2a318&originHeight=68&originWidth=489&originalType=binary&ratio=1&rotation=0&showTitle=false&size=47905&status=done&style=none&taskId=u4ec1dd5f-6a35-42fd-8584-f361e6118d0&title=&width=580)<br />当执行update或者delete后，分区目录的变化如下，比原来的目录多了all_1_1_0_3目录和all_2_2_0_3目录，以及一个mutation_3.txt文件。<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/390265/1703643831523-1a9ba4e9-c918-4e8a-bec3-44c094d13f32.png#averageHue=%23262422&clientId=u833a1b5e-6319-4&from=paste&height=112&id=u987fc977&originHeight=112&originWidth=574&originalType=binary&ratio=1&rotation=0&showTitle=false&size=74316&status=done&style=none&taskId=ub90b583d-fcf3-4ff9-82a6-6053f02821a&title=&width=574)<br />![image.png](https://cdn.nlark.com/yuque/0/2023/png/390265/1703644426775-9a7cfb6b-0e37-428e-9ed8-073d5c768ab6.png#averageHue=%233a3933&clientId=u833a1b5e-6319-4&from=paste&height=78&id=u9f549f66&originHeight=66&originWidth=486&originalType=binary&ratio=1&rotation=0&showTitle=false&size=49249&status=done&style=none&taskId=u23ac98e1-91eb-490a-93cb-23b2ca1c2ee&title=&width=571)<br />新目录的命名规则是在原有名称上加上system.mutations.block_numbers.number。数据在重写的过程中会将需要删除的数据去掉。旧的数据目录并不会立即删除，而是会被标记成非激活状态（active为0）。等到MergeTree引擎的下一次合并动作触发时，这些非激活目录才会被真正从物理意义上删除。<br />那么后缀的数字从何而来呢？查询system.mutations系统表
```plsql
SELECT database, table ,mutation_id, block_numbers.number as num ,is_done FROM system.mutations
```
![image.png](https://cdn.nlark.com/yuque/0/2023/png/390265/1703644347786-8721e65b-6ddf-4b78-ac87-aa137ec05df5.png#averageHue=%23494138&clientId=u833a1b5e-6319-4&from=paste&height=70&id=ua32dba50&originHeight=70&originWidth=556&originalType=binary&ratio=1&rotation=0&showTitle=false&size=46836&status=done&style=none&taskId=u4f0c8fc0-6a1d-402f-a862-206511905a7&title=&width=556)<br />我们要ALTER的表是person_update。从表中看到num是3，所以后缀就是3，is_done表示操作已经完成。
<a name="GDYEy"></a>
## 数据写入
